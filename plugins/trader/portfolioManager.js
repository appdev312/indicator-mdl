...
var Manager = function(conf) {
  ...
  // conf.fund - Initial fund allocated for pair
  // this.currentFund - Current trade amount for pair
  this.fund = parseFloat(conf.fund);
  this.currentFund = parseFloat(conf.currentFund);
...
Manager.prototype.setPortfolio = function(callback) {
  var set = function(err, fullPortfolio) {
    ...
    if(_.isEmpty(this.portfolio))
      this.emit('portfolioUpdate', this.convertPortfolio(portfolio, this.fund));
...
Manager.prototype.trade = function(what, retry) {
  ...
  var act = function() {
    ...
    if(what === 'BUY') {
      if (this.currentFund <= this.getBalance(this.currency)) {
        amount = this.currentFund / this.ticker.ask;
      } else {
        amount = 0;
      }

      console.log('** portfolioManager.trade BUY : ', amount, this.asset);
      if (amount > 0) {
      ...
      console.log('** portfolioManager.trade SELL : ', amount, this.asset);
      if (amount > 0) {
...
Manager.prototype.convertPortfolio = function(portfolio, altCurrency = null) {
...
Manager.prototype.relayOrder = function(done) {
  ...
  var relay = (err, res) => {
    ...
    async.series([
      ...
      const portfolio = this.convertPortfolio(this.portfolio, this.currentFund);
