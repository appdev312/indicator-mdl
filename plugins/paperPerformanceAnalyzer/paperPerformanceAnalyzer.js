...
console.log("plugins/paperPerformanceAnalyzer/paperPerformanceAnalyzer: ", config.watch);
const perfConfig = config.paperPerformanceAnalyzer;
...
const PaperPerformanceAnalyzer = function() {
  _.bindAll(this);
...
PaperPerformanceAnalyzer.prototype.processCandle = function(candle, done) {
...
PaperPerformanceAnalyzer.prototype.paperProcessPortfolioUpdate = function(portfolio) {
  this.start = portfolio;
  this.current = _.clone(portfolio);
}

PaperPerformanceAnalyzer.prototype.paperProcessTrade = function(trade) {
  this.trades++;
...
PaperPerformanceAnalyzer.prototype.logRoundtripPart = function(trade) {
...
PaperPerformanceAnalyzer.prototype.round = function(amount) {
  return amount.toFixed(8);
}

PaperPerformanceAnalyzer.prototype.handleRoundtrip = function() {
  var roundtrip = {
...
PaperPerformanceAnalyzer.prototype.calculateReportStatistics = function() {
...
PaperPerformanceAnalyzer.prototype.finalize = function(done) {
  const report = this.calculateReportStatistics();
  this.handler.finalize(report);
  done();
}

module.exports = PaperPerformanceAnalyzer;
